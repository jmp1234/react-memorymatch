...
class Cards extends React.Component {
  ...
  render() {
    ...
    const {indexes_clicked} = this.props.cardInfo

    const card_front_style = {
      'backgroundImage': `url(${imageName}),  linear-gradient(315deg, #b8c6db 0%, #f5f7fa 74%)`
    }
    const card_back_style = indexes_clicked.indexOf(index) >= 0 ? {display: 'none'} : {};
    return(
      <div onClick={() => {card_clicked(link, index)}}className="card">
